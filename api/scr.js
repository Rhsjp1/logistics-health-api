...
async function getTrafficScore(origin, dest) {
  const token = process.env.MAPBOX_TOKEN;

  if (!token) {
    return {
      score: 70,
      raw: { provider: "fallback", reason: "Missing MAPBOX_TOKEN" }
    };
  }

  const coords = `${origin.lon},${origin.lat};${dest.lon},${dest.lat}`;

  const trafficUrl = `https://api.mapbox.com/directions/v5/mapbox/driving-traffic/${coords}?overview=false&annotations=duration&access_token=${token}`;
  const baseUrl = `https://api.mapbox.com/directions/v5/mapbox/driving/${coords}?overview=false&annotations=duration&access_token=${token}`;

  const [trafficRes, baseRes] = await Promise.all([fetch(trafficUrl), fetch(baseUrl)]);

  if (!trafficRes.ok || !baseRes.ok) {
    return {
      score: 65,
      raw: {
        provider: "fallback",
        reason: `Mapbox error traffic=${trafficRes.status} base=${baseRes.status}`
      }
    };
  }
  ...
  const trafficDuration = trafficData.routes?.[0]?.duration;
  const baseDuration = baseData.routes?.[0]?.duration;

  if (!trafficDuration || !baseDuration) {
    return {
      score: 65,
      raw: { provider: "fallback", reason: "Missing route durations" }
    };
  }

  const ratio = trafficDuration / baseDuration;
  const delayPct = (ratio - 1) * 100;

  let score = 100;
  score -= clamp(delayPct * 2.2, 0, 60);
  ...
  return {
    ...
    raw: {
      ...
      base_duration_min: Math.round(baseDuration / 60),
...
export default async function handler(req, res) {
  try {
    ...
    const [originWeather, destWeather, traffic] = await Promise.all([
      ...
      getTrafficScore(originLoc, destLoc)
    ]);
    ...
    return res.status(200).json({
      ...
      meta: {
        ...
        scoring_version: "v1-live",
        data_sources: {
          weather: "Open-Meteo",
          traffic: traffic.raw.provider === "mapbox" ? "Mapbox Directions" : "Fallback",
